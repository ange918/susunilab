      ...
      <main>
        ...
        <section className="relative h-screen flex items-center justify-center overflow-hidden bg-[#1B2441]">
          ...
          >
            ...
            >
              Laboratoire stratégique, culturel et innovant dédié à l'émergence des nouveaux imaginaires africains.
            </motion.p>
            <motion.div 
            ...
            >
              <a href="#branches" className="px-10 py-5 bg-[#0078B7] text-white rounded-2xl font-bold transition-all hover:scale-105 active:scale-95 shadow-2xl shadow-[#0078B7]/30">
                Explorer nos branches
              </a>
              <Link href="/a-propos" className="px-10 py-5 border-2 border-white/20 hover:border-white text-white rounded-2xl font-bold transition-all">
                À propos de nous
              </Link>
            </motion.div>
          </motion.div>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col md:flex-row items-center gap-16">
              <motion.div 
                ...
                viewport={{ once: true }}
              >
                <h2 className="text-4xl md:text-6xl font-bold text-[#1B2441] mb-8 leading-tight">Un laboratoire au cœur de <span className="text-[#0078B7]">l'innovation africaine</span></h2>
                ...
                <Link href="/a-propos" className="text-[#0078B7] font-bold inline-flex items-center hover:underline group text-lg">
              ...
              <motion.div 
                ...
                viewport={{ once: true }}
              >
                 <div className="text-[#1B2441]/10 font-black text-6xl rotate-12 select-none">VISION</div>
        ...
        <section id="branches" className="py-32 bg-gray-50">
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {branches.map((branch, idx) => (
              ...
              >
                <div className="text-[#0078B7] mb-8 transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">{branch.icon}</div>
                <h3 className="text-2xl font-bold mb-4 text-[#1B2441] leading-tight">{branch.title}</h3>
                <p className="text-gray-600 text-lg leading-relaxed mb-8 flex-grow">{branch.description}</p>
                <Link href={branch.link} className="text-[#0078B7] font-bold inline-flex items-center hover:underline group/btn">
                  En savoir plus
            ...
            <div className="space-y-6">
              {[
                ...
                <motion.div 
                  ...
                  transition={{ delay: idx * 0.1 }}
                >
                  <Disclosure as="div" className="bg-gray-50 rounded-[2rem] overflow-hidden border border-gray-100">
                    <DisclosureButton className="group flex w-full items-center justify-between p-8 text-left focus:outline-none hover:bg-gray-100 transition-colors">
                      <span className="text-xl font-bold text-[#1B2441]">{item.q}</span>
        ...
        <section id="projets" className="py-32 bg-white">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h2 className="text-4xl md:text-6xl font-bold text-[#1B2441] mb-6 tracking-tight">RÉALISATIONS</h2>
              <p className="text-gray-500 max-w-md text-xl leading-relaxed">Un aperçu des projets qui définissent notre approche du design, de la tech et de la culture.</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, x: 30 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
            >
              <Link href="/projets" className="text-[#0078B7] font-black uppercase tracking-[0.2em] text-sm flex items-center group">
                Tous les projets <ChevronRightIcon className="ml-2 w-5 h-5 group-hover:translate-x-2 transition-transform" />
              </Link>
            </motion.div>
          </div>
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { title: "Expo 2025 Cotonou", type: "Culture & Design", bg: "bg-gray-100" },
              { title: "Digital Heritage Lab", type: "Innovation", bg: "bg-gray-200" },
              { title: "Strategie Territoriale", type: "Consulting", bg: "bg-gray-300" }
            ].map((p, idx) => (
              <motion.div 
                key={idx}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 p-10 text-white translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <span className="text-xs uppercase tracking-[0.3em] mb-3 block font-bold text-[#0078B7]">{p.type}</span>
                  <h4 className="text-3xl font-black tracking-tight">{p.title}</h4>
                  <button className="mt-6 px-6 py-2 border border-white/30 rounded-full text-sm hover:bg-white hover:text-[#1B2441] transition-colors">Explorer</button>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        {/* COLLABORER SECTION */}
        <section id="collaborer" className="py-40 bg-gray-50 relative overflow-hidden">
          <div className="max-w-5xl mx-auto px-6 relative z-10 text-center">
            <motion.div 
              ...
              viewport={{ once: true }}
            >
              <div className="inline-block px-6 py-2 bg-blue-100 text-[#0078B7] rounded-full text-sm font-black uppercase tracking-widest mb-10">Partenariats</div>
              <h2 className="text-4xl md:text-7xl font-black text-[#1B2441] mb-10 leading-none tracking-tighter">PRÊT À RÉINVENTER <br />L'AVENIR WITH NOUS ?</h2>
              <p className="text-xl md:text-2xl text-gray-600 mb-16 max-w-3xl mx-auto leading-relaxed font-light">Nous collaborons with des institutions, des marques et des créatifs qui partagent notre soif d'excellence et d'authenticité radicale.</p>
              <Link href="/collaborer" className="inline-flex items-center justify-center px-12 py-6 bg-[#1B2441] hover:bg-[#0078B7] text-white rounded-2xl font-black text-xl transition-all shadow-2xl hover:-translate-y-2 group">
